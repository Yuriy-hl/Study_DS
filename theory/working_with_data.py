...
from istatistics import correlation
...
from istatistics import standard_deviation

def scale(data: List[Vector]) -> Tuple[Vector, Vector]:
    """Возвращает среднее значение и станд отклонение для каждой позиции"""
    dim = len(data[0])
...
def rescale(data: List[Vector]) -> List[Vector]:
    ...
    (оставляет позицию как есть, если ее сто равно 0)
    """
    dim = len(data[0])
    means, stdevs = scale(data)

    # Сделать копию для каждого вектора
